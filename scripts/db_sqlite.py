...
class BD_SQLite_Celulas(IDataSource):
    ...
    def get_celulas_activas(self):
        sql ="""
                SELECT id_celula, cod_red, c_lider, c_lider_red, nombre_lider, cod_base, anfitriones, direccion 
    ...
    def get_liderazgo_redes(self):
        return read_sql('SELECT * FROM V_LIDERAZGO_X_COD_RED_ACTIVO', self.conexion)
    
    def get_temas(self):
        return read_sql('SELECT * FROM temas ORDER BY id_tema DESC', self.conexion)
